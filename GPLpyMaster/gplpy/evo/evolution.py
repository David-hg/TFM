...
class Evolution(object):
    ...
    def tournament_selection(self, battle_size=5):
        ...
        tournament = []
        for i in range(self.parents_pool_size):
            combatientes = np.random.choice(self.population, size=battle_size, replace=False)
            tournament = np.concatenate((tournament, combatientes), axis=None)
...
Setup.__new__.__defaults__ = ('GUPI+ED', Evolution_EDA, 100, ProbabilisticModel.uniform, EDA, OnePointMutation, .05, 0.001, .5, 100, 0.5, 0.25, 0.0, 30, .005, False, 10, 0.02, 10)
        ...
        for s in self.setups:
            ...
            for x in range(0, self.samples):
                ...
                if self.logger:
                    ...
                    experiments_id.append([x, fit, avg_git, it, l_it])
